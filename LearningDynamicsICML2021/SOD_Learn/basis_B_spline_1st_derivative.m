function yprime = basis_B_spline_1st_derivative(x, eta, p, knots)
% yprime = basis_B_spline_1st_derivative(x, eta, p, knots)
...
yprime       = zeros(size(x));
% find the derivative of B-spline polynomial by its order
if p > 0 % derivative of degree 0 B-splines are zero
...
% yprime is p * B_{eta, p - 1}(x)/(x_{eta + p} - x_eta)
    yprime   = p * B_spline(x, eta, p - 1)/(knots(eta + p) - knots(eta));
  elseif knots(eta) == knots(eta + p) && knots(eta + 1) < knots(eta + p + 1)   
% yprime is -p * B_{eta + 1, p - 1}(x)/(x_{eta + p + 1} - x_{eta + 1})
    yprime   = -p * B_spline(x, eta + 1, p - 1)/(knots(eta + p + 1) - knots(eta + 1));
  else
% use the derivative B-spline recursion formula
    yprime   = p * (B_spline(x, eta, p - 1)/(knots(eta + p) - knots(eta)) - ...
      B_spline(x, eta + 1, p - 1)/(knots(eta + p + 1) - knots(eta + 1)));
  end
end