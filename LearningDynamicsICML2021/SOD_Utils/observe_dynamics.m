...
if ~isfield(obs_info,'time_vec') || isempty(obs_info.time_vec)
  if obs_info.L == 1
    obs_info.time_vec = obs_info.T_0;                                                               % when we only want one observation, we take the initial time
  else
    obs_info.time_vec = linspace(obs_info.T_0, obs_info.T, L);                                      % default to (equi-distance) times in [0, obs_info.T]
  end
else
...
try
  [traj, dtraj]       = dense_output(solution, obs_info.time_vec);                                  % find out the trajectory position and derivative at those time instances
