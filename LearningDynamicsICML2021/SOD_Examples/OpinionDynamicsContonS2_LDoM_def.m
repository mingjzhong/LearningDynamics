function Example = OpinionDynamicsContonS2_LDoM_def()
% function Example = OpinionDynamicsContonS2_LDoM_def()
% Define associated terms for OPINION DYNAMICS Continuous on Sphere (S^2)

% (c) M. Zhong

% System
sys_info.name                = 'OpinionDynamicsContonS2';                                           % name of the dynamics
sys_info.d                   = 3;                                                                   % dimension for the state vector (in this case, opinion vector)
sys_info.N                   = 20;                                                                  % # of agents
sys_info.R                   = 5/pi;                                                                   % radius of S^1/S^2
sys_info.IR                  = sys_info.R * pi;
sys_info.phiE                = {@(r) OD_phiE(r, 5, sys_info.IR)};                                   % energy based interaction
...
sys_info.weight_on_manifold  = @(x) geodesic_direction_on_sphere(x, x, sys_info.d, sys_info.R);
sys_info.mu0                 = @() sphere_init_config(sys_info.d, sys_info.N, sys_info.R);          % distribution of initial conditions
