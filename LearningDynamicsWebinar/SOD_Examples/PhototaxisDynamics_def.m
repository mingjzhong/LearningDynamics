...
sys_info.phiA            = {@(r) PT_phiA(r, beta, sys_info.N)};                                     % alignment based interaction
...
sys_info.I_0             = 0.1;                                                                     % light intensity
sys_info.v_term          = [60; 0];                                                                 % terminal velocity (light direction at infinity)
sys_info.xi_critical     = 0.3;                                                                     % critical excitation level (for light effect to activate)
sys_info.gamma           = @(xi, cutoff) PT_smooth_cutoff(xi, cutoff);                              % the smooth cutoff function on light effect on xi and v
sys_info.Fv              = @(x, v, xi) PT_Fv(v, xi, sys_info.d, sys_info.N, ...
                           sys_info.gamma, sys_info.xi_critical, sys_info.I_0, sys_info.v_term);    % non-collective change to v
sys_info.phiXi           = {@(r) PT_phiA(r, beta, sys_info.N)};                                     % interactional kernel on xi_i
sys_info.xi_capacity     = 2 * sys_info.xi_critical;                                                % excitation capacity (for light effect to decay)
sys_info.Fxi             = @(x, v, xi) PT_Fxi(xi, sys_info.I_0, sys_info.xi_capacity, ...
                           sys_info.gamma);                                                         % non-collective change on xi_i
...
% Observations
obs_info.L               = 500;                                                                     % observe (equi-spaced) times for learning interaction kernel(s)
obs_info.M               = 250;                                                                     % # trajectories with random initial conditions for learning interaction kernel(s)
