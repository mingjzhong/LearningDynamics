...
% hold off;
bar(1 : sys_info.N, mean(massErrs, 1), 0.8);
massStr             = cellfun(@(x) num2str(x, '%0.1e'), num2cell(sys_info.known_mass), ...
                      'UniformOutput', false);
xticklabels_new     = cell(1, sys_info.N);
for idx = 1 : sys_info.N
  xticklabels_new{idx} = strtrim(sprintf('%s\\newline%s\n', sys_info.AO_names{idx}, massStr{idx}));
end
% for idx = 1 : sys_info.N
%   text(BH.XEndPoints(idx), 1.015 * BH.YEndPoints(idx), num2str(sys_info.known_mass(idx), '%0.1e'), ...
%     'FontName', plot_info.tick_font_name, 'FontSize', plot_info.tick_font_size, ...
...
axis([1 - 0.5, sys_info.N + 0.5, y_min * 0.9, y_max * 1.1]);
%legendHandle        = legend([BH1, BH2], {'True Masses', 'Estimated Masses'});
% legendHandle        = legend(BH, {'Relative Errors for Mass Estimation'});
...
TL                   = title('Rel. Err. in Learning Masses', 'interpreter', 'latex', 'FontName', ...
                       plot_info.title_font_name, 'FontSize', plot_info.title_font_size);
