...
elseif contains(sys_info.name, 'PowerLaw') || contains(sys_info.name, 'TanhLaw')
  sys_info.N          = sys_info.N * N_ratio;
  sys_info.type_info  = ones(1, sys_info.N);
  sys_info.mu0        = @() CP_init_config(sys_info.name, sys_info.d, sys_info.N);
...
elseif contains(sys_info.name, 'PhototaxisDynamics')
  ...
  sys_info.Fv         = @(x, v, xi) PT_Fv(v, xi, sys_info.d, sys_info.N, sys_info.gamma, ...
                        sys_info.xi_critical, sys_info.I_0, sys_info.v_term);                       
  sys_info.Fxi        = @(x, v, xi) PT_Fxi(xi, sys_info.I_0, sys_info.xi_capacity, sys_info.gamma);                     
end
end