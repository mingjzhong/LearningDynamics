...
for time_ind = 1 : length(time_vec)
  for ind = 1 : length(trajs)
    ...
    lg_handles(ind, :)           = plot_one_point_1D(traj_fig, sp_handle, trajs{ind}(:, time_ind), ...
                                   time_vec(time_ind), dtrajs{ind}(:, time_ind), ...
                                   arrow_lens{ind}(:, time_ind), T_loc == time_ind, ...
                                   the_sys_info, plot_info);
    ...
    if mod(ind, 2) == 1
      ylabel('Coord. $1$', 'Interpreter', 'latex', 'FontSize', plot_info.axis_font_size, ...
        'FontName', plot_info.axis_font_name);
    end
    if ind == length(trajs) - 1 || ind == length(trajs)
      xlabel('Time $t$', 'Interpreter', 'latex', 'FontSize', plot_info.axis_font_size, ...
        'FontName', plot_info.axis_font_name);
    end
