...
if isfield(learn_info, 'debug_dy_app') && learn_info.debug_dy_app
  M                              = size(obs_data.y, 3);
  appErrs                        = zeros(1, M);
  for m = 1 : M
    appErrs(m)                   = norm(squeeze(dy(:, :, m) - obs_data.dy_true(:, :, m)), Inf);
  end
  fprintf('\nApproximate derivative is %10.4e%c%10.4e away from truth', mean(appErrs), 177, ...
    std(appErrs));
end
if learn_info.VERBOSE >= 1
  fprintf('\n--------------------------------------------------------------------------------');
