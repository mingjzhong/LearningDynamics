function basis = construct_basis_Ck1Ck2_ND(phi_range, knots, basis_info)
% function basis = construct_basis_Ck1Ck2_ND(phi_range, knots, basis_info)

% (C) M. Zhong
       
basis.degree                    = basis_info.degree;
basis.supp                      = phi_range;
basis.dim                       = size(phi_range, 1);
basis.type                      = basis_info.type;
if isfield(basis_info, 'sub_type') && ~isempty(basis_info.sub_type)
...
%     basis.d2f                   = basis1Ds{1}.d2f;
  ...
  otherwise
    error('SOD_Learn:construct_basis_Ck1Ck2:exception', ...
      'The dim. of a basis function cannot be more than 3!!');
end
end