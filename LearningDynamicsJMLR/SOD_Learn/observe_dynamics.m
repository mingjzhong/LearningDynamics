...
if ~isfield(obs_info,'time_vec') || isempty(obs_info.time_vec)
    if obs_info.L == 1
        ...
        obs_info.time_vec      = linspace(0.001, obs_info.T_L, L);                                      % default to (equi-distance) times in [0,obs_info.T_L]
    end
else
...
try
    ...
    if isfield(obs_info,'obs_noise') && obs_info.obs_noise>0
        traj = obs_info.mu_trajnoise(traj);
        dtraj= obs_info.mu_dtrajnoise(dtraj);
    end
catch
    traj            = NaN;
    dtraj           = NaN;
