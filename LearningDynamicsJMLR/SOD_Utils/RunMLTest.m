...
SAVE_DIR                = '~/DataAnalyses/LearningDynamics/outputs';                                % Please keep this fixed, simply create a symlink ~/DataAnalyses pointing wherever you like
VERBOSE                 = 0;
    ...
    for Midx = 1:size(learningOutput{Lidx},1)
        for nidx = 1:size(learningOutput{Lidx},2)
            for k = 1:size(learningOutput{Lidx},3)
                try
                    Err_Rel_EErr(Midx,Lidx,nidx,k,:,:) = learningOutput{Lidx}{Midx,nidx,k}.EErrSmooth.Rel;
                catch
                end
                try
                    Err_Rel_AErr(Midx,Lidx,nidx,k,:,:) = learningOutput{Lidx}{Midx,nidx,k}.AErrSmooth.Rel;
                catch
                end
                try
                    Err_Rel_XiErr(Midx,Lidx,nidx,k,:,:) = learningOutput{Lidx}{Midx,nidx,k}.XiErrSmooth.Rel;
                catch
                end
            end
        end
...
save( sprintf('%s/%s_learningOutputAll_%s_%s_.mat',SAVE_DIR,sys_info.name,datestr(datetime('now')),char(getHostName(java.net.InetAddress.getLocalHost))),...
    '-v7.3','sys_info','solver_info','obs_info','learn_info','learningOutput','Err_Rel_EErr' );
fprintf('done.');

%% figures
for k_1 = 1:size(Err_Rel_EErr,5)
    for k_2 = 1:size(Err_Rel_EErr,6)
        if exist('Err_Rel_EErr')
            try
                figure;imagesc(flipud(min(mean(Err_Rel_EErr(:,:,:,:,k_1,k_2),4),[],3)));colorbar;set(gca,'YTickLabels',fliplr(M_plane));set(gca,'XTickLabels',L_plane);title(sprintf('Interaction E-kernel (%d,%d)',k_1,k_2));
                figure;imagesc(flipud(log10(min(mean(Err_Rel_EErr(:,:,:,:,k_1,k_2),4),[],3))));colormap(gray);colorbar;set(gca,'YTickLabels',fliplr(M_plane));set(gca,'XTickLabels',L_plane);title(sprintf('Interaction E-kernel (%d,%d)',k_1,k_2));
            catch
            end
        end
        if exist('Err_Rel_AErr')
            try
                figure;imagesc(flipud(min(mean(Err_Rel_AErr(:,:,:,:,k_1,k_2),4),[],3)));colorbar;set(gca,'YTickLabels',fliplr(M_plane));set(gca,'XTickLabels',L_plane);title(sprintf('Interaction A-kernel (%d,%d)',k_1,k_2));
                figure;imagesc(flipud(log10(min(mean(Err_Rel_AErr(:,:,:,:,k_1,k_2),4),[],3))));colormap(gray);colorbar;set(gca,'YTickLabels',fliplr(M_plane));set(gca,'XTickLabels',L_plane);title(sprintf('Interaction A-kernel (%d,%d)',k_1,k_2));
            catch
            end
        end
        if exist('Err_Rel_XiErr')
            try
                figure;imagesc(flipud(min(mean(Err_Rel_XiErr(:,:,:,:,k_1,k_2),4),[],3)));colorbar;set(gca,'YTickLabels',fliplr(M_plane));set(gca,'XTickLabels',L_plane);title(sprintf('Interaction Xi-kernel (%d,%d)',k_1,k_2));
                figure;imagesc(flipud(log10(min(mean(Err_Rel_XiErr(:,:,:,:,k_1,k_2),4),[],3))));colormap(gray);colorbar;set(gca,'YTickLabels',fliplr(M_plane));set(gca,'XTickLabels',L_plane);title(sprintf('Interaction Xi-kernel (%d,%d)',k_1,k_2));
            catch
            end
        end
    end
