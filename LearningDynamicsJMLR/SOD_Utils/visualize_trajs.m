...
trajs                   = cell(1, 4);
result                  = construct_and_compute_traj(plot_info.solver_info, sys_info, sys_info_approx, ICs, obs_info.T_L, obs_info.L_test);
trajs{1}                = result.traj_true;
trajs{2}                = result.traj_hat;
time_vec                = result.time_vec;
if strcmp(sys_info.name, 'PredatorPrey1stOrder')
...
if plot_info.for_larger_N
  ...
  if isfield(learningOutput{1}, 'y_init_new')
    ...
    ICs                 = generateICs(obs_info.M_test, sys_info);
  end
end
result                  = construct_and_compute_traj(plot_info.solver_info, sys_info, sys_info_approx, ICs, obs_info.T_L, obs_info.L_test);
