...
% (c) M. Zhong
% add the special trial for unbounded lennard jones example by Sui


if strcmpi(sys_info.name,'LennardJonesDynamics')
for m = 1 : size(ICs, 2)
  y_init           = ICs(:, m);%deval(dynamics,obs_info.time_vec(1))
  dynamics         = self_organized_dynamics(y_init, sys_info, solver_info);
  dynamicshat      = self_organized_dynamics(deval(dynamics,0.001), syshat_info, solver_info);
  if ~dynamics.flag && ~dynamicshat.flag, break; end
end
c                  = floor(solver_info.time_span(2)/T_L);
time_vec           = linspace(0.001, solver_info.time_span(2), c * L - c + 1);
