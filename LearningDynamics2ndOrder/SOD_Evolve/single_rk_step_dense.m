...
if stage == 1
  ...
  switch order
    ...
    case 4
      ...
      switch degree
        case 2
          for ind = 1 : length(b_vec)
            btheta_vec(ind) = 3 * (2 * c_vec(ind) - 1) * b_vec(ind) * theta^2 + ...
                              2 * (2 - 3 * c_vec(ind)) * b_vec(ind) * theta;
          end
        case 3
          btheta_vec(1)     = 2 * (1 - 4 * b_vec(1)) * theta^3 + ...
                              3 * (3 * b_vec(1) - 1) * theta^2 + theta;
          for ind = 2 : length(b_vec)
            btheta_vec(ind) = 4 * (3 * c_vec(ind) - 2) * b_vec(ind) * theta^3 + ...
                              3 * (3 - 4 * c_vec(ind)) * b_vec(ind) * theta^2;
          end
        otherwise
      end
    otherwise
  ...
  for s_idx = 2 : stage
    ...
    y_stage(:, s_idx)       = ode_fun(t_n + h * c_vec(s_idx), ...
                              y_n + h * sum(y_stage(:, 1 : s_idx - 1) * A_diag, 2)); 
  end
