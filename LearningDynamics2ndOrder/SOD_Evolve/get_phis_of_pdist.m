...
for k1 = 1 : sys_info.K                                                                                 
  ...
  for k2 = 1 : sys_info.K                                                                           % for the influence of C_k2 agents on C_k1 agents        
    ...
    phi_of_pdist_Ck1_Ck2                = evaluate_phi_of_pdist(phis{k1, k2}, pdist_data{k1, k2});
...
if nnz(ind) > 0
  error('SOD_Evolve:get_phis_of_pdist:exception', 'phi(0) = Inf!!');
end
ind                                     = isnan(phis_of_pdist);
if nnz(ind) > 0
  error('SOD_Evolve:get_phis_of_pdist:exception', 'phi(0) = NaN!!');
end
end