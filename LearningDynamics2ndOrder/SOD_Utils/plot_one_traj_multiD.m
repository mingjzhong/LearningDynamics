...
for k = 1 : sys_info.K
  ...
  for agent_ind = 1 : N_k
    ...
    if sys_info.d < 3, c3_at_t = []; end   
    if sys_info.d == 2
      p_handle = patch([c1_at_t, NaN], [c2_at_t, NaN], [plot_info.c_vecs{k}, NaN * ones(1, 1, 3)], ...
                 'EdgeColor', 'interp', 'LineStyle', plot_info.line_styles{line_style_ind}, ...
                 'LineWidth', plot_info.traj_line_width);
    else
      p_handle = patch([c1_at_t, NaN], [c2_at_t, NaN], [c3_at_t, NaN], ...
                 [plot_info.c_vecs{k}, NaN * ones(1, 1, 3)], 'EdgeColor', 'interp', 'LineStyle', ...
                 plot_info.line_styles{line_style_ind}, 'LineWidth', plot_info.traj_line_width);
      view(3); % default line of sight in 3D                                                                                     
    end
