...
[x_mean, x_std, v_mean, v_std, xi_mean, xi_std, y_mean, y_std]   = getTrajErrStat(trajErrs, ...
                                                                   sys_info);
type                                                             = 'training IC''s';
printTrajErrs(obs_info, solver_info, x_mean, x_std, v_mean, v_std, xi_mean, xi_std, y_mean, ...
  y_std, type);
...
if isfield(learningOutput{1}, 'trajErr_Ntransfer') && ~isempty(learningOutput{1}.trajErr_Ntransfer)
  for ind = 1 : length(learningOutput)
    trajErrs{ind}                                                = learningOutput{ind}.trajErr_Ntransfer;
  end
  [x_mean, x_std, v_mean, v_std, xi_mean, xi_std, y_mean, y_std] = getTrajErrStat(trajErrs, sys_info);
  type                                                           = 'larger N';
  printTrajErrs(obs_info, solver_info, x_mean, x_std, v_mean, v_std, xi_mean, xi_std, y_mean, ...
    y_std, type);
end
