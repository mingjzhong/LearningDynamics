function printTrajErrs(obs_info, solver_info, x_mean, x_std, v_mean, v_std, xi_mean, xi_std, ...
         y_mean, y_std, type)
...
for idx = 1 : err_kind
  switch idx
    case 1
      fprintf('\n\tsup-norm  on [T_0, T] with T_0 = %10.4e and T = %10.4e:', ...
        obs_info.time_vec(1), obs_info.T); 
    case 2
  ...
  if ~isempty(xi_mean)
    fprintf('\n\t\tFor xi: mean = %10.4e%c%10.4e, std = %10.4e%c%10.4e.', ...
      mean(xi_mean(idx, :)), 177, std(xi_mean(idx, :)), mean(xi_std(idx, :)), 177, ...
      std(xi_std(idx, :)));
  end 
  if ~isempty(y_mean)
    fprintf('\n\t\tFor  y: mean = %10.4e%c%10.4e, std = %10.4e%c%10.4e.', ...
      mean(y_mean(idx, :)), 177, std(y_mean(idx, :)), mean(y_std(idx, :)), 177, std(y_std(idx, :)));    
  end
end
end