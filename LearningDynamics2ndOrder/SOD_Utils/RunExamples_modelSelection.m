...
if ExampleIdx == 1 || ExampleIdx == 2
  for idx = 1 : n_trials
    ...
    obs_data                    =  generateObservations(sys_info, solver_info, obs_info, ...
                                   obs_info.M, []);
    ...
    fprintf('\nThe empirical error from learning phis on x/v is: %10.4e.', ...
      learn_out.Estimator.emp_err);  
    obs_info.rhoLT              = learn_out.Estimator.rhoLTM;  
    fprintf('\nComputing corresponding L^2(rho^L_T) norms of estimators...');
    learn_out.L2rhoTE           = relativeErrorInfluenceFunction(learn_out.Estimator.phiEhat, ...
                                  sys_info.phiE, sys_info, obs_info, learn_out.Estimator.Ebasis, ...
                                  'energy');
    learn_out.L2rhoTA           = relativeErrorInfluenceFunction(learn_out.Estimator.phiAhat, ...
                                  sys_info.phiA, sys_info, obs_info, learn_out.Estimator.Abasis, ...
                                  'alignment');
    fprintf('\nThe L^2(rho^L_T) of the learned estimator for phi^E is: %12.6e.', ...
      learn_out.L2rhoTE.Rel);
    fprintf('\nThe L^2(rho^L_T) of the learned estimator for phi^A is: %12.6e.', ...
      learn_out.L2rhoTA.Rel);
    learningOutput{idx}         = learn_out;
  end
  fprintf('\n=========================After %d trials', n_trials);
  fprintf('\nThe L^2(rho^L_T) of the learned estimator for phi^E is: %12.6e%c%12.6e.', ...
    mean(cellfun(@(x) x.L2rhoTE.Rel, learningOutput)), 177, std(cellfun(@(x) x.L2rhoTE.Rel, ...
    learningOutput)));
  fprintf('\nThe L^2(rho^L_T) of the learned estimator for phi^A is: %12.6e%c%12.6e.', ...
    mean(cellfun(@(x) x.L2rhoTA.Rel, learningOutput)), 177, std(cellfun(@(x) x.L2rhoTA.Rel, ...
    learningOutput)));
  plot_info.plot_name           = sprintf('%s/%s_learningOutput_%s',SAVE_DIR, sys_info.name, ...
                                  time_stamp);
  displayMSLearningResults(learningOutput, sys_info, plot_info);
%% Save
  save(sprintf('%s/%s_learningOutput%s.mat', SAVE_DIR, sys_info.name, time_stamp), '-v7.3', ...
    'sys_info', 'solver_info', 'obs_info', 'learn_info', 'obs_data', 'learningOutput', 'plot_info');  
elseif ExampleIdx == 3 || ExampleIdx == 4
  for idx = 1 : n_trials
    ...
    elseif ExampleIdx == 4
      ...
      xs                          = obs_data.y(1 : block, :, :);
      vs                          = obs_data.y(block + 1 : 2 * block, :, :);
    end
    fprintf('done (%.2f sec).',obs_data.Timings.total);
  % Learn the dynamics from observations
    ...
    output1                       = LearnAndComputeTrajectoryErr(obs_info, solver_info, ...
                                    learn_info, obs_data);
    fprintf('\nLearn and Compute the Trajectory Errors.');
    learn_info.sys_info.ode_order = 2;
    obs_data.x                    = [xs; vs];
    obs_data.ICs                  = [squeeze(xs(:, 1, :)); squeeze(vs(:, 1, :))];
    output2                       = LearnAndComputeTrajectoryErr(obs_info, solver_info, ...
                                    learn_info, obs_data);
    learningOutput{idx}.output1st = output1;
    learningOutput{idx}.output2nd = output2;
  end
  fprintf('\n=========================After %d trials', n_trials);
  fprintf('\nTrajectory error, 1st order, for t in [%10.4e,%10.4e]:', 0, obs_info.T);
  fprintf('\nmean = %10.4e%c%10.4e, std = %10.4e%c%10.4e', ...
    mean(cellfun(@(x) mean(x.output1st.trajErr), learningOutput)), 177, ...
    std(cellfun(@(x) mean(x.output1st.trajErr), learningOutput)), ...
    mean(cellfun(@(x) std(x.output1st.trajErr), learningOutput)), 177, ...
    std(cellfun(@(x) std(x.output1st.trajErr), learningOutput)));
  fprintf('\nTrajectory accuracies, 2nd order, for t in [%10.4e,%10.4e]:', 0, obs_info.T);
  fprintf('\nmean = %10.4e%c%10.4e, std = %10.4e%c%10.4e', ...
    mean(cellfun(@(x) mean(x.output2nd.trajErr), learningOutput)), 177, ...
    std(cellfun(@(x) mean(x.output2nd.trajErr), learningOutput)), ...
    mean(cellfun(@(x) std(x.output2nd.trajErr), learningOutput)), 177, ...
    std(cellfun(@(x) std(x.output2nd.trajErr), learningOutput)));
%% Save
