function phis_of_pdist = find_phis_of_pdist(pdist_mat, theta_mat, sys_info, type)
% function phis_of_pdist = find_phis_of_pdist(pdist_mat, theta_mat, sys_info, type)
...
%   phi_of_pdist_Ck1_Ck2                = phis{k1, k2}(pdist_mat(row_ind, col_ind));
    ...
    if isfield(sys_info, 'has_theta') && sys_info.has_theta
      theta_Ck1_Ck2_mat                 = theta_mat(row_ind, col_ind);
      theta_Ck1_Ck2_vec                 = theta_Ck1_Ck2_mat(:);
      phi_of_pdist_Ck1_Ck2              = reshape(phis{k1, k2}(pdist_Ck1_Ck2_vec, theta_Ck1_Ck2_vec), [N_k1, N_k2]) ;  % evaluate phi_{k_1, k_2}(r_{i, i'}) with i \in C_k1, j \in C_k2 
    else
