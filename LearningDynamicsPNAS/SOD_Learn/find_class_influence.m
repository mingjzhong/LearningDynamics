function class_influence = find_class_influence( psis, pdist_data, regulation, pdiff_data, d, num_agents, kappa, basis_info, ISSPARSE)
...
if nargin<9, ISSPARSE   = false; end
...
%  if DEBUG, Timings.Code1 = tic; end
  ...
  if ISSPARSE
    ...
    class_influence                         = zeros(sz_class_influence);                        % allocate storage for class_influence, vs and vprimes
  end
  ...
  for k = 1:length(populatedBins)                                                                 % go through bins in distance space
    ...
    for kp = 1:length(f_idxs)
      ...
      if ~isempty(regulation)                                                                 % add regulation factor
        reg_wrt_pdist_sorted                 = regulation(pdist_data_sorted_idxs(idxs));
        psi_of_r_idxs                        = psi_of_r_idxs .* reg_wrt_pdist_sorted;
      end
      ...
      for p = 1:d                                                                             % multiply basis function by difference vectors
        ...
        if ISSPARSE
          ...
          class_influence(p:d:end,f_idxs(kp)) = sum(infl_tmp_mat, 2);                                % sum over the class C_{k_2} for all i' in C_{k_2} and scaled by the modifier
        end
      end
    end
  ...
  if ISSPARSE
    ...
    class_influence  = sparse(i,j,s,sz_class_influence(1),sz_class_influence(2));
  end
