function collective_change = get_collective_change(x, v, xi, pdist_data, pdiff_data, agent_info, sys_info, kind)
% function collective_change = get_collective_change(x, v, xi, pdist_data, pdiff_data, agent_info, sys_info, kind)
%   calculates \sum_{i' \in K_i'} 1/N_K_i' * \phi_{K_i, K_i'}(|x_i - x_i'|) * pdiff_ii'
%   where pdiff_ii' = x_i - x_i', v_i - v_i', or xi_i - xi_i', or 1 which is given in pdiff_data variable
% IN:
%
% OUT:
...
if ~isempty(phi_res), phis_of_pdist = phis_of_pdist .* phis_res; end                                % restriction is applied entry-wise   
