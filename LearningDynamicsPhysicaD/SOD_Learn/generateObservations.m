...
if ~comp_traj, traj = obs_data.traj; else, traj = cell(M, 1); end

parfor m = 1:M
    if comp_traj, traj{m} = self_organized_dynamics (ICs(:, m), sys_info, solver_info); end         % run the dynamics    
    flag(m)                     = traj{m}.flag;
    if flag(m), continue; end
    [y_obs(:, :, m), dobstraj, ~, Timingsobsdynamics(m)] = observe_dynamics(traj{m}, obs_info);     % extract observations from trajectories stopping at T_L
    if obs_info.use_derivative, dy_obs(:, :, m) = dobstraj;                                     end
end
