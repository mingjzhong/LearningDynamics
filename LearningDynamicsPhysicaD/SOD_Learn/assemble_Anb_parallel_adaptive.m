function [Estimator, Timings] = assemble_Anb_parallel_adaptive(x_d, v_d, xi_d, dot_xv_d, dot_xi_d, time_vec, sys_info, learn_info)
% function [Estimator, Timings] = assemble_Anb_parallel_adaptive(x_d, v_d, xi_d, dot_xv_d, dot_xi_d, time_vec, sys_info, learn_info)
% OUT:
...
spmd
  if labindex == 1, num_cores_l = numlabs; else, num_cores_l = []; work_cpu_list = []; end          % Lab(1) is in charge of communicating, so it has to know the number of workers in the system    
  x = getLocalPart(x_d); v = getLocalPart(v_d); xi = getLocalPart(xi_d); dot_xv = getLocalPart(dot_xv_d); 
  dot_xi = getLocalPart(dot_xi_d);                                                                  % get local copies of v, xi, dot_xv and dot_x
...
% initialize some indicators                                    
    agent_info = getAgentInfo(sys_info); M_l = size(x, 3);
    ...
    basis_l                                  = construct_basis_uniform(phi_range_l, sys_info, learn_info);
    ...
    ALM_l = zeros(A_cols.total); bLM_l = zeros(A_cols.total, 1); rhs_in_l2_norm_sq_l = 0;           % save the square of l_2 norm of d_vec - F_vec over all m's
    if A_cols.Xi > 0
      ALMXi_l = zeros(A_cols.Xi); bLMXi_l = zeros(A_cols.Xi, 1); rhs_in_l2_norm_sq_xi_l = 0; 
    else
      ALMXi_l = []; bLMXi_l = []; rhs_in_l2_norm_sq_xi_l = [];
    end
    ...
    for m = 1 : M_l                                                                                 % start the MC loop
      ...
      learn_info.temp_dir, sys_info.name, learn_info.time_stamp, labindex, m);      
...
M = sum([M_l{:}]); L = length(time_vec); Estimator.ALMXi = []; Estimator.bLMXi = [];
