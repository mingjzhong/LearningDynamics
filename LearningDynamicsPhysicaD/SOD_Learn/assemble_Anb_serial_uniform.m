function [Estimator, Timings] = assemble_Anb_serial_uniform(x, v, xi, dot_xv, dot_xi, time_vec, sys_info, learn_info)
% function [Estimator, Timings] = assemble_Anb_serial_uniform(x, v, xi, dot_xv, dot_xi, time_vec, sys_info, learn_info)
% OUT:
...
basis                                                   = construct_basis(phi_range, sys_info, learn_info);
...
if isfield(learn_info, 'use_gpu') && ~isempty(learn_info.use_gpu) && learn_info.use_gpu
  Estimator.ALM                                         = gpuArray(sparse(A_cols.total, A_cols.total));
  Estimator.bLM                                         = gpuArray(sparse(A_cols.total, 1));
else  
  Estimator.ALM                                         = zeros(A_cols.total); 
  Estimator.bLM                                         = zeros(A_cols.total, 1); 
end
rhs_in_l2_norm_sq                                       = 0; % save the square of l_2 norm of d_vec - F_vec over all m's
Estimator.ALMXi                                         = []; 
Estimator.bLMXi                                         = []; 
if A_cols.Xi > 0
  Estimator.ALMXi                                       = zeros(A_cols.Xi); 
  Estimator.bLMXi                                       = zeros(A_cols.Xi, 1); 
  rhs_in_l2_norm_sq_xi                                  = 0; 
else
  Estimator.ALMXi                                       = []; 
  Estimator.bLMXi                                       = []; 
  rhs_in_l2_norm_sq_xi                                  = [];   
end
...
for m = 1 : M                                                                                       % start the MC loop
  ...
  if ~isempty(F_vec_m)
    if isfield(learn_info, 'use_gpu') && ~isempty(learn_info.use_gpu) && learn_info.use_gpu
      rhs_vec                                           = gpuArray(d_vec_m - F_vec_m);
    else
      rhs_vec                                           = d_vec_m - F_vec_m;
    end
  else
    if isfield(learn_info, 'use_gpu') && ~isempty(learn_info.use_gpu) && learn_info.use_gpu
      rhs_vec                                           = gpuArray(d_vec_m);
    else
      rhs_vec                                           = d_vec_m;
    end    
  end
  if isfield(learn_info, 'use_gpu') && ~isempty(learn_info.use_gpu) && learn_info.use_gpu
    Psi                                                 = gpuArray([PsiE_m, PsiA_m]);
  else
    Psi                                                 = [PsiE_m, PsiA_m];  
  end
  rhs_in_l2_norm_sq                                     = rhs_in_l2_norm_sq + norm(rhs_vec)^2/(M * L);
  Estimator.ALM                                         = Estimator.ALM + (transpose(Psi) * Psi)/(M * L); % assemble the approximated Estimator.ALM matrix
  Estimator.bLM                                         = Estimator.bLM + (transpose(Psi) * rhs_vec)/(M * L);  
  if ~isempty(Estimator.ALMXi)
    if ~isempty(Fxi_vec_m), rhs_vec_xi = dxi_vec_m - Fxi_vec_m; else, rhs_vec_xi = dxi_vec_m; end
    rhs_in_l2_norm_sq_xi                                = rhs_in_l2_norm_sq_xi + norm(rhs_vec_xi)^2/(M * L);  
    Estimator.ALMXi                                     = Estimator.ALMXi + (transpose(PsiXi_m) * PsiXi_m)/(M * L);
    Estimator.bLMXi                                     = Estimator.bLMXi + (transpose(PsiXi_m) * rhs_vec_xi)/(M * L);
  else
