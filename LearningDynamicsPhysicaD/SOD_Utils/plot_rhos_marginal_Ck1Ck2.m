function [rhoPlotHandles, rhoPlotNames, eff_range] = plot_rhos_marginal_Ck1Ck2(fig_handle, ax, range, rhoLT, rhoLTMs, k1, k2, sys_info, plot_info)
% function [rhoPlotHandles, rhoPlotNames] = plot_rhos_marginal_Ck1Ck2(fig_handle, ax, range, rhoLT, rhoLTMs, k1, k2, sys_info, plot_info)
...
RPH_count                   = 0;
centers                     = linspace(range(1), range(2), plot_info.rho_resolution(k1, k2));
n_trials                    = length(rhoLTMs);
rhoLTMs_ctrs                = zeros(n_trials, length(centers));
if ~isempty(rhoLT)
  RPH_count                 = RPH_count + 1;
  histdata1                 = rhoLT(centers);
  eff_range{RPH_count}      = get_effective_range(histdata1, centers);
...
if ~isempty(rhoLTMs)
  ...
  eff_range{RPH_count}      = get_effective_range(histdata2, centers);
...
if length(ax.YAxis) > 1
  ...
  ax.YAxis.Exponent        = get_exponent_scale(tmpmax);
