...
for ind = 1 : total_num_trials
  ...
  rhoLTME                                  = find_the_rhoLT(learningOutput{ind}.rhoLTM, 'energy');
  rhoLTM{ind}                              = rhoLTME.rhoLTR{1, 1};
end
plot_info.display_interpolant              = false;
plot_phis_and_rhos(fig_handle, axis_handle, [], rhoLTM, phiE, phiEhats, [], 1, 1, sys_info, plot_info);
...
plot_interactions_and_rhos(phi_fig, axis_handle, [], rhoLTM, phiA, phiAhats, [], 1, 1, sys_info, plot_info);
xlabelHandle                               = xlabel('$r$ (pairwise distance)', 'FontSize', plot_info.axis_font_size);
...
rhoLTMR                                    = find_the_rhoLT(learningOutput{1}.rhoLTM, 'R');
rhoLTMDR                                   = find_the_rhoLT(learningOutput{1}.rhoLTM, 'DR');
rhoLTMRDR                                  = find_the_rhoLT(learningOutput{1}.rhoLTM, 'RDR');
plot_one_joint_distribution(rho_fig, rhoLTMR, rhoLTMDR, rhoLTMRDR, 1, plot_info);
% save the figure
