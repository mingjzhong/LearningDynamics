...
if learn_info.is_parallel
  ...
  if isempty(gcp('nocreate'))
    ...
    pool                                     = parpool('local', num_workers, 'IdleTimeout', ...
                                               time_out, 'SpmdEnabled', true);
  else
    delete(gcp('nocreate'))
    num_workers                              = feature('numcores');                                 % actual physical cores
    pool                                     = parpool('local', num_workers, 'IdleTimeout', ...
...
fprintf('\n================================================================================');
fprintf('\nGenerating rhoT......');
[obs_info.rhoLT, rhoLT_timings]              = generateRhoLT(sys_info, solver_info, obs_info);
...
%% Learn the interactions  
  ...
  learn_out                                  = learningRoutine(solver_info, obs_info, learn_info); % Learning
  obs_data                                   = learn_out.obs_data;
  learn_out.obs_data                         = [];
...
%% Test performance on trajectories
  ...
  if isfield(sys_info, 'LC_type') && ~isempty(sys_info.LC_type), find_LC = true; else, find_LC = false; end
  syshat_info                                = learn_out.syshatsmooth_info;
  [learn_out.trajErr, learn_out.trajErr_new, learn_out.trajErr_Ntransfer, learn_out.syshat_info_Ntransfer] ...
                                             = estimateTrajAccuracies(sys_info, syshat_info, sys_info_Ntransfer, ...
                                               obs_data.ICs, obs_info, solver_info, find_LC);                              % Testing performance on trajectories
  learn_out.Timings.estimateTrajAccuracies   = toc(learn_out.Timings.estimateTrajAccuracies);
  fprintf('\n------------------- Time for Trajectory Error: %.2f', learn_out.Timings.estimateTrajAccuracies);
...
%% Test pattern indicators
  if isfield(sys_info, 'LC_type') && ~isempty(sys_info.LC_type)
    ...
    fprintf('\n------------------- Time for Pattern Indicators: %.2f', learn_out.Timings.estimatePattIndicators);
  end
...
%% Display & figures
if VERBOSE >= 1
  final_visualization(learningOutput, obs_info, solver_info, sys_info, sys_info_Ntransfer, learn_info, ...
    plot_info);
end
