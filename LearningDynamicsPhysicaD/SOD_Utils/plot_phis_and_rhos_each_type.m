function plot_phis_and_rhos_each_type(win_handler, range, rho_range, phi, phihats, phihatsmooths, ...
         rhoLTR, rhoLTMRs, rhoLTX, rhoLTMXs, sys_info, plot_info)
% function plot_phis_and_rhos_each_type(win_handler, range, rho_range, phi, phihats, phihatsmooths, ...
% rhoLTR, rhoLTMRs, rhoLTX, rhoLTMXs, sys_info, plot_info)
...
n_trials = length(phihats); phihatsCk1Ck2 = cell(1, n_trials); phihatsmoothsCk1Ck2 = cell(1, n_trials);
rhoLTMRsCk1Ck2 = cell(1, n_trials); rhoLTMXsCk1Ck2 = cell(1, n_trials);
...
for k1 = 1 : sys_info.K
  for k2 = 1 : sys_info.K
    if strcmp(plot_info.phi_type, 'energy') || is_SOD
      axis_handle                        = subplot(sys_info.K, sys_info.K, (k1 - 1) * sys_info.K + k2); 
    else
    ...
    for ind = 1 : n_trials
      ...
      phihatsmoothsCk1Ck2{ind}           = phihatsmooths{ind}{k1, k2};
      rhoLTMRsCk1Ck2{ind}                = rhoLTMRs{ind}{k1, k2};
    end
    plot_phis_and_rhos_each_type_Ck1Ck2(win_handler, axis_handle, range{k1, k2}, rhoLTRCk1Ck2, rhoLTMRsCk1Ck2, ...
      phiCk1Ck2, phihatsCk1Ck2, phihatsmoothsCk1Ck2, k1, k2, sys_info, plot_info);
    if k1 == sys_info.K
      ...
      if is_SOD
        ylabel('$\xi$ (pairwise $\xi_{i, i''}$)','FontSize', plot_info.axis_font_size, 'FontName', plot_info.axis_font_name, ...
        'Interpreter', 'latex');        
      end
    end
    if plot_info.showplottitles
      titleHandle = title(sprintf('N = %d, T = %.0f, M = %d, L = %d, n = %d', sys_info.N, obs_info.T_L, ...
        obs_info.M, obs_info.L, basis_info.n(k_1,k_2)), 'FontSize', plot_info.title_font_size);
      set(titleHandle, 'Position', get(titleHandle,'Position') .* [1, 0.95, 1]);
      set(titleHandle, 'Interpreter', 'Latex', 'FontSize', plot_info.title_font_size,'FontName', plot_info.title_font_name, ...
        'FontWeight', 'bold' );
    end    
    if strcmp(plot_info.phi_type, 'energy'), handleAxes(k1, k2) = axis_handle; else, handleAxes(k1, 2 * k2 - 1) = axis_handle; end
    if ~strcmp(plot_info.phi_type, 'energy') && ~is_SOD
      ax                                 = subplot(sys_info.K, 2 * sys_info.K, (k1 - 1) * 2 * sys_info.K + 2 * k2); 
      yyaxis right;
      ...
      [rhoPlotHandles, rhoPlotNames]     = plot_rhos_marginal_Ck1Ck2(win_handler, ax, rho_range{k1, k2}, rhoLTXCk1Ck2, rhoLTMXsCk1Ck2, ...
