...
data_kind                     = 3;                                                                  % use either daily or houly data
...
degrees                       = [1, 2, 3];
num_sub_ints                  = [20, 30, 40, 50];
overall_time                  = tic;
fprintf('\nTesting Learning of JPL %s data with varying degrees.', time_type);
for idx1 = 1 : length(num_sub_ints)
  num_sub_int                 = num_sub_ints(idx1);
  for idx2 = 1 : length(degrees)
    degree                    = degrees(idx2);
    learn_info.Ebasis_info    = set_JPL_basis_info(learn_info.Ebasis_info, num_sub_int, ...
                                         degree);
...
%% trajectory errors and other planet information 
    ...
    dynamics_type             = 'Smooth Phihats';
    [~, ~, trajErr, dynamics] = get_JPL_trajErr(obs_data, sys_info, solver_info, syshat_info, ...
                                num_years, total_num_years, dynamics_type);          
    learn_out.trajErr         = trajErr;
    learn_out.dynamicshat     = dynamics;
    Timings.trajErr           = toc(Timings.trajErr);
    fprintf('\n------------------- Time for traj. err.: %6.2f sec.', Timings.trajErr);
    learn_out.Timings         = Timings;
    save_file                 = sprintf('%s/LD_degree%d_subint%d_hourly_%s.mat', SAVE_DIR, ...
                                degree, num_sub_int, time_stamp);
    save(save_file, '-v7.3', 'learn_out', 'learn_info');
  end
end
