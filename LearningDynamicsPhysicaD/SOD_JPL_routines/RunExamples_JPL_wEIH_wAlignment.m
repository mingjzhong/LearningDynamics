...
[Example, obs_data]            = Gravitation_JPL_def(selection_idx, num_years, total_num_years, ...
                                 use_v, data_kind);
fprintf('\nIt takes %6.2f secs to load the data.\n', toc(load_data_time));
...
find_EIH_acce_time             = tic;
option.method                  = 1;
fprintf('\nFinding EIH based Acceleration with method = %d.', option.method);
dy_EIH                         = get_JPL_EIH_acceleration(sys_info, solver_info, obs_data, option);
obs_data.dy                    = dy_EIH;                                                            % use the EIH acceleration
fprintf('\nIt takes %6.2f to find the acceleration.', toc(find_EIH_acce_time));
learn_time                     = tic;
fprintf('\nLearning over 400 years with EIH acceleration and with alignment');
num_sub_int                    = 50;
degree                         = 3;
...
[trajs, ~, trajErr, dyn_hat]   = get_JPL_trajErr(obs_data, sys_info, solver_info, syshat_info, ...
                                 num_years, total_num_years, dynamics_type);
oneBlock                       = sys_info.N * sys_info.d;    
PI                             = get_JPL_planet_information_over_time(trajs{2}(1 : oneBlock, :), ...
                                 obs_data.time_vec, sys_info);  
fprintf('\nThe Precession rate of Mercury is: %10.4e.', PI(1, 4, 1));
fprintf('\nIt takes %6.2f secs to calculate traj. err.', toc(trajErr_time));
%% save the output
save_file                      = sprintf('%s/%s_%s_wEIH_wAlign.mat', SAVE_DIR, sys_info.name, ...
                                 time_stamp);
save(save_file, '-v7.3', 'sys_info', 'solver_info', 'obs_data', 'SAVE_DIR', 'syshat_info', ...
  'num_years', 'total_num_years', 'learn_out', 'dyn_hat', 'dyn_EIH', 'trajs', 'PI');
%% Done
fprintf('\nDone.\n');
return
