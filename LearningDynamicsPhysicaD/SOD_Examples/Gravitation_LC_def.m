...
AO_names                    = {'Sun',   'Mercury', 'Venus', 'Earth', 'Mars', 'Jupiter', 'Saturn', 'Uranus', 'Neptune', 'Pluto'}; % names of the Astronomical Objects (AO) in our Solar system
AO_mass                     = [1.9885e6,    0.330,    4.87,    5.97,  0.642,      1898,      568,     86.8,       102, 0.0146];  % NASA data, in unit: 10^24 kg, 
perihelions                 = [0,            46.0,   107.5,   147.1,  206.6,     740.5,   1352.6,   2741.3,    4444.5, 4436.8];  % NASA data, in unit: 10^6 km
aphelions                   = [0,            69.8,   108.9,   152.1,  249.2,     816.6,   1514.5,   3003.6,    4545.7, 7375.9];  % NASA data, in unit: 10^6 km
orbitalPeriods              = [0,            88.0,   224.7,   365.2,  687.0,      4331,    10747,   30688.5,    60182,  90560];  % NASA data, in unit: day
% orbitalSpeeds               = [0,            47.4,      35,    29.8,   24.1,      13.1,      9.7,      6.8,       5.4,    4.7] * 8.64e-2;  % NASA data, in unit: 10^6 km/day
...
obs_info.M_rhoT             = 500;                                                                 % # trajectories with random initial conditions to compute approximate \rho_T
...
for k1 = 1 : sys_info.K
  for k2 = 1 : sys_info.K
    switch type{1}
      case 'B-spline'
        if k1 ~= k2 && basis_info{k1, k2}.n < (num_sub_ints + degree)
          basis_info{k1, k2}.n = num_sub_ints + degree;
        end
      case 'PW-polynomial'
        if k1 ~= k2 && basis_info{k1, k2}.n < num_sub_ints * (degree + 1)
          basis_info{k1, k2}.n = num_sub_ints * (degree + 1);
        end      
      otherwise
    end
  end
